...
class Particle:
    def __init__(self, n_clusters, data, use_MeanShifts=True, w=0.72, c1=1.49, c2=1.49):
        self.n_clusters = n_clusters
        if use_MeanShifts:
            MeanShift = MeanShift(n_clusters=self.n_clusters)
            MeanShift.fit(data)
            self.centroids_pos = MeanShift.cluster_centers_
        else:
